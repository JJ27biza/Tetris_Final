...
public class Xogo {
    ...
    public void borrarLinasCompletas(int y) {
        ...
        numeroLinas++;
        this.ventanaPrincipal.getPuntuacionLabel().setText(numeroLinas * 10 + " puntos");
        if(numeroLinas%5==0){
        ventanaPrincipal.timer.setDelay(ventanaPrincipal.timer.getDelay()-numeroLinas*5);}
        cambiarCadrados(y);

    }

    private boolean finalXogo() {
        boolean fin = false;
        for (int i = 0; i < cadradosChan.size(); i++) {
            if (cadradosChan.get(i).y <= 60) {
